...
events in which one $\Pgt$ lepton decays hadronically and one leptonically (``semi-leptonic'' $\Pgt$ pair decays),
...
d\Phi_{n}= 
 ...
 & \mbox{if } \Pgt^{+} \to \textrm{hadrons} + \Pnut \mbox{ and } \Pgt^{-} \to \textrm{hadrons} + \Pnut \\
   d\Phi^{(1)}_{\ellnunu} \, d\Phi^{(2)}_{\tauhnu} \, 
 & \mbox{if } \Pgt^{+} \to \ellnunu \mbox{ and } \Pgt^{-} \to \textrm{hadrons} + \Pnut \\
   d\Phi^{(1)}_{\tauhnu} \, d\Phi^{(2)}_{\ellnunu} \, 
 & \mbox{if } \Pgt^{+} \to \textrm{hadrons} + \Pnut \mbox{ and } \Pgt^{-} \to \ellnunu \\
   d\Phi^{(1)}_{\ellnunu} \, d\Phi^{(2)}_{\ellnunu} \, 
 & \mbox{if } \Pgt^{+} \to \ellnunu \mbox{ and } \Pgt^{-} \to \ellnunu \, ,
 \end{cases}
...
d\Phi^{(i)}_{\ellnunu} = & \frac{d^{3}\bm{\hat{p}}^{\vis(i)}}{(2\pi)^{3} \, 2
  \hat{E}_{\vis(i)}} \, \frac{d^{3}\bm{\hat{p}}^{\Pnu(i)}}{(2\pi)^{3} \, 2
  \hat{E}_{\Pnu(i)}} \, \frac{d^{3}\bm{\hat{p}}^{\APnu(i)}}{(2\pi)^{3} \, 2
  \hat{E}_{\APnu(i)}} \, .
...
the factor $\frac{1}{2 \, x_{a} \, x_{b} \, s}$, referred to as ``flux factor'' in the literature,
...
squared moduli $\vert \mathcal{M}^{(1)}_{\Pgt}(\bm{\tilde{p}}) \vert^{2}$ and $\vert \mathcal{M}^{(2)}_{\Pgt}(\bm{\tilde{p}}) \vert^{2}$ of the ME for the $\Pgt$ lepton decays
...
The dimension of the integration over the phase-space elements $d^{3}\bm{p}^{\Pnu(i)}$ and $d^{3}\bm{p}^{\Pnu(i)} \, d^{3}\bm{p}^{\APnu(i)}$
can be reduced by analytic transformations.
...
so that the magnitude of the $\bm{p}^{\inv}$ vector is given by $\sqrt{\left( \frac{1 - z}{z} \right)^{2} \, E^{2}_{\vis} - m^{2}_{\inv}}$ in that case.
With the convention that $m_{\inv} = 0$ for hadronic $\Pgt$ decays,
Eqs.~(\ref{eq:hadTauDecaysCosTheta}) and~(\ref{eq:lepTauDecaysCosTheta}) can be expressed by a common form that is valid for hadronic as well as leptonic $\Pgt$ decays:
\begin{equation}
\cos\theta_{\inv} = \frac{\frac{1 - z}{z} \, E^{2}_{\vis} \, E_{\inv} - \frac{1}{2}(m^{2}_{\Pgt} - (m^{2}_{\vis} + m^{2}_{\inv}))}{\vert\bm{p}^{\vis}\vert \, 
...
The axis of the cone is given by the $\bm{p}^{\vis}$ vector and its opening angle is given by Eq.~(\ref{eq:theta_inv}).
...
\caption{
  ...
  relative to the direction of the visible $\Pgt$ decay products, given by the vector $\bm{p}^{\vis}$.
}
...
\vert \mathcal{M}^{(i)}_{\Pgt} \vert^{2} \, d\Phi^{(i)}_{\tauhnu} 
 = & \, \frac{\pi}{m_{\Pgt}\Gamma_{\Pgt}}(\bm{\tilde{p}}) \,
 f_{\Phadron}\left(\bm{\tilde{p}}^{\vis(i)}, m^{\vis(i)},
   \bm{\tilde{p}}^{\inv(i)}\right) \, \frac{d^{3}\bm{\hat{p}}^{\vis}}{2 \hat{E}_{\vis}} \, dz \, d\phi_{\inv} \nonumber \\
\vert \mathcal{M}^{(i)}_{\Pgt}(\bm{\tilde{p}}) \vert^{2} \, d\Phi^{(i)}_{\ellnunu} 
 = & \, \frac{\pi}{m_{\Pgt}\Gamma_{\Pgt}} \, f_{\ell}\left(\bm{\tilde{p}}^{\vis(i)},
 m^{\vis(i)}, \bm{\tilde{p}}^{\inv(i)}\right) \, \frac{d^{3}\bm{\hat{p}}^{\vis}}{2 \hat{E}_{\vis}} \, dz \, dm^{2}_{\inv} \, d\phi_{\inv}
 \, .
\label{eq:PSint}
\end{align}
...
The momenta $\bm{\tilde{p}}^{\vis(i)}$ and $\bm{\tilde{p}}^{\inv(i)}$ in the zero transverse momentum frame of the $\PHiggs$ boson
...
\mathcal{M}^{(1)}_{\Pgt}(\bm{\tilde{p}}) \vert^{2} \, d\Phi^{(1)} \, \vert
\mathcal{M}^{(2)}_{\Pgt}(\bm{\tilde{p}}) \vert^{2} \, d\Phi^{(2)}$ according to
Eq.~(\ref{eq:PSint}).
...
\end{equation}
is proportional to the inverse of $\vert \BW_{\PHiggs} \vert^{2}$, 
the squared modulus of the Breit-Wigner propagator of the $\PHiggs$ boson in
Eq.~(\ref{eq:meHiggsBreitWigner})~\footnote{
