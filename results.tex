...
The value of $\kappa$ has been chosen such that the optimal resolution on $m_{\Pgt\Pgt}$ is achieved in each channel.
...
We take the difference in peak position between the mass distributions reconstructed by the different algorithms
into account by quantifying their resolution in terms of the ratio
...
by scaling the output of the algorithm by the ratio of the true mass to the median obtained before the calibration is applied.
...
The relative resolution on $m_{\Pgt\Pgt}$, quantified by the ratio $\sigma/\textrm{M}$,
is actually worse for the CA method compared to the relative resolution on $m_{\vis}$.
...
In all three decay channels, the SVfitMEM algorithm provides a substantial improvement in the relative mass resolution.
...
on the expense of increasing the low mass tails in the $m_{\Pgt\Pgt}$ distribution reconstructed in signal events.
...
potential high mass signals would likely be buried underneath the high mass tail of the background otherwise.
...
We would like to add two further remarks:
...
products is typically high in case of hadronic $\Pgt$ decays and low in case of leptonic $\Pgt$ decays.
